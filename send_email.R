...
foSendEmail <- function(category, name, org, email, message, version, app_name = "RokaiApp", sendername = "RokaiServer"){
  if(!EMAIL_AVAILABLE){
  ...
  tryCatch({
    ...
    library(mailR)
    send.mail(from = paste0(sendername, " <", email_credentials$username, "@gmail.com>"),
